...
if (document.getElementById('image-preview') != null) {
	...
	const imageDefaultText = imageContainer.querySelector('.image-preview-default');
...
updateBtn.forEach(el => {
	el.addEventListener('click', function () {
		document.querySelector('#update-modal input[name="nama"]').value = this.dataset.nama;
		const url = document.querySelector('#update-modal').action;
		document.querySelector('#update-modal').action = url + '/' + this.dataset.id;
	});
})
...
updateBtnLoc.forEach(el => {
	el.addEventListener('click', function () {
		document.querySelector('#update-modal input[name="lat"]').value = this.dataset.lat;
		document.querySelector('#update-modal input[name="lng"]').value = this.dataset.lng;
		document.querySelector('#update-modal input[name="namaLokasi"]').value = this.dataset.namalokasi;
		document.querySelector('#update-modal input[name="alamat"]').value = this.dataset.alamat;
		Array.from(document.querySelector('#update-modal select[name="apotik"]').options)
			.forEach(elOpt => {
				if (elOpt.value == this.dataset.apotik) {
					elOpt.selected = true;
				}
			})
		Array.from(document.querySelector('#update-modal select[name="wifi"]').options)
			.forEach(elOpt => {
				if (elOpt.value == this.dataset.wifi) {
					elOpt.selected = true;
				}
			})
		Array.from(document.querySelector('#update-modal select[name="ac"]').options)
			.forEach(elOpt => {
				if (elOpt.value == this.dataset.ac) {
					elOpt.selected = true;
				}
			})
		Array.from(document.querySelector('#update-modal select[name="ruang_asi"]').options)
			.forEach(elOpt => {
				if (elOpt.value == this.dataset.ruangasi) {
					elOpt.selected = true;
				}
			})
		Array.from(document.querySelector('#update-modal select[name="ruang_tunggu"]').options)
			.forEach(elOpt => {
				if (elOpt.value == this.dataset.ruangtunggu) {
					elOpt.selected = true;
				}
			})
		Array.from(document.querySelector('#update-modal select[name="nebulizer"]').options)
			.forEach(elOpt => {
				if (elOpt.value == this.dataset.nebulizer) {
					elOpt.selected = true;
				}
			})
		Array.from(document.querySelector('#update-modal select[name="kelurahan"]').options)
			.forEach(elOpt => {
...
updateBtnKec.forEach(el => {
	el.addEventListener('click', function () {
		document.querySelector('#update-modal input[name=kecamatan]').value = this.dataset.nama;
		const url = document.querySelector('#update-modal').action;
		document.querySelector('#update-modal').action = url + '/' + this.dataset.id;
	});
})
...
updateBtnKel.forEach(element => {
	element.addEventListener('click', function (e) {
		document.querySelector('#update-modal input[name="kelurahan"]').value = this.dataset.kelurahan;
		Array.from(document.querySelector('#update-modal select[name="kecamatan"]').options)
			.forEach(elOpt => {
